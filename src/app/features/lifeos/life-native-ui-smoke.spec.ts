...
describe('LifeOS native UI options', () => {
  it('provides colored priority, focus, energy and context picker options', () => {
    ...
    expect(lifePriorityPickerOptions(DEFAULT_LIFE_OS_CONFIG).every((o) => !!o.color)).toBeTrue();
    ...
    expect(lifeContextPickerOptions(DEFAULT_LIFE_OS_CONFIG.requirements, 'build').every((o) => !!o.icon && !!o.color)).toBeTrue();
  });
});
